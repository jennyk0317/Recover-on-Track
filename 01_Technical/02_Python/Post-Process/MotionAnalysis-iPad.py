...
def main(): 
    ...
    #load metadata
    metadata_path = 'C:\\Users\\jihyk\\Dropbox (Partners HealthCare)\\Recover-on-Track\\01_Technical\\02_Python\\iPad-record3d\\'


    with open(metadata_path + 'metadata-td.json') as user_file:
    ...
    results = np.load(filename + '_3D_clean_pxl.npy', allow_pickle=True) #load previously saved list of coordinates from pose estimation of video
    ...
    for i in range(time.shape[0]): 
        ...
        for j in range(17):
            joint_3D_pxl[j,0] = landmarks[j,0]
            joint_3D_pxl[j,1] = landmarks[j,1]
            joint_3D_pxl[j,2] = landmarks[j,2]
