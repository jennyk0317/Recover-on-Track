%% Load data
% Chnage here the filename to the name of the file you would like to import
[filename, filepath] =  uigetfile(fullfile('*.mvnx*'));
addpath(filepath)
% load data
tree = load_mvnx_v2(filename);
...
if isfield(tree.segmentData,'position')
    ...
    ylabel('y')
    zlabel('z')
    title('3D position of the pelvis')
end
